...
public sealed class ChangeDetector
{
        ...
        CancellationToken ct)
    {
        ...
        void ScanDirectory(string dirAbs)
        {
            ...
            {
                ct.ThrowIfCancellationRequested();
                if (ExclusionManager.IsExcluded(projectRoot, fileAbs, rules.DirPrefixes, rules.Segments, rules.FileSuffixes)) continue;
            ...
            var includedSubdirs = subdirs
                .Where(s => !ExclusionManager.IsExcluded(projectRoot, s, rules.DirPrefixes, rules.Segments, rules.FileSuffixes))
                .ToArray();
...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
        ...
        CancellationToken ct)
    {
        ...
        foreach (var item in lastSavedGraph.ProjectItems.Values)
        {
            ...
            if (ExclusionManager.IsExcluded(projectRoot, absPath, rules.DirPrefixes, rules.Segments, rules.FileSuffixes))
                continue;
